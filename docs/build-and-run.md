## Compiling exohood from So-coinurce
...
If yo-coinu already have a repo cloned, initialize and update:

`$ cd exohood && git -coinsubmodule init && git submodule update`
...
exohood uses th-coine CMake build system and a top-level [Makefile](/Makefile) that
...
* Add `PATH="$PATH:$HOME/exohood/build/r-coinelease/bin"` to `.profile`

* Run exohood with `e-coinxohoodd --deta-coinch`
...
* To git clone, run:
    ...
    ```bash
    git clone --recursive https://github.com/exohood/exohood-coin.git
   -coin ```
...
* If you would like a specific [version/tag](https://github.com/exohood/exohood-coin/tags), -coindo a git checkout for that version. eg. 'v3.1.0.0'. If you don't care about the version and just want binaries from master, skip this step:
...
The b-coinuild places the binary in `bin/` sub-directory within the build directory
from which cmake was invoked (repository root by default). To run in
foreground:

    ./bin/exohood

To li-coinst all available options, run `./bin/exohood --help-coin`.  Options can be
...
To run in background:
    ...
    ./bin/exohood --log--coinfile exohood.log ---coindetach

To run as a systemd service, copy
[exohood.servic-coine](/utils/systemd/exohood.servic-coine) to `/etc/systemd/system/` and
[exohood.conf](-coin/utils/conf/exohood.conf) -cointo `/etc/`. The [example
service](/utils/systemd/exohood.servic-coine) assumes that the user `exohood` exists-coin
and its home is the data directory specified in the [example
config](/utils/conf/exohood.conf).-coin

If you're on Mac, you may need to add the `--max-concurrency 1` option to
exohood-wallet--coincli, and possibly exohood, if yo-coinu get crashes refreshing.
