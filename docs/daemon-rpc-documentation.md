...
This is a list of the Exohood daemon RPC calls, their inputs and outputs, and examples of each.

Many RPC calls use the daemon's JSON RPC interface while others use their own interfaces, as demonstrated below.

Note: "Atomic Units" refer to the smallest fraction of 1 EXO according to the Exohood implementation.  **1 EXO = 100 atomic units**
...
The majority of Exohood RPC calls use the daemon's  `json_rpc`  interface to request various bits of information. These methods all follow a similar structure, for example:
...
```
$ curl -X POST http://127.0.0.1:26968/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_template","params":{"wallet_address":"EXOkKzqmNPQMk7DehZMif4FLJa1hpUQXjH6ULFEmyVUkRfJfrbphCZsCQt8G2dq1hbC2h4QsDMeohMrUeHcTmtSF4uG2NL756Z","reserve_size":60}' -H 'Content-Type: application/json'
...
* *block_header*  - A structure containing block header information.
  ...
  * *difficulty*  - unsigned int; The strength of the Exohood network based on mining power.
  * *hash*  - string; The hash of this block.
  * *height*  - unsigned int; The number of blocks preceding this block on the blockchain.
  * *major_version*  - unsigned int; The major version of the Exohood protocol at this block height.
  * *minor_version*  - unsigned int; The minor version of the Exohood protocol at this block height.
  * *nonce*  - unsigned int; a cryptographic random one-time number used in mining a Exohood block.
  ...
  * *reward*  - unsigned int; The amount of new Atomic Units generated in this block and rewarded to the miner. Note: 1 EXO = 100 atomic units.
...
In this example, blocks range from height 1545999 to 1546000 is looked up:
...
For calls that end with  **.bin** , the data is exchanged in the form of binary, serialized objects, as defined in the [Core RPC Server](https://github.com/Exohood/Exohood/blob/master/src/rpc/core_rpc_server_commands_defs.h).
...
* *overspend*  - boolean; Transaction uses more EXO than available ( `true` ) or not ( `false` ).
...
```
$ curl -X POST http://127.0.0.1:26968/start_mining -d '{"do_background_mining":false,"ignore_battery":true,"miner_address":"EXOkKzqmNPQMk7DehZMif4FLJa1hpUQXjH6ULFEmyVUkRfJfrbphCZsCQt8G2dq1hbC2h4QsDMeohMrUeHcTmtSF4uG2NL756Z","threads_count":1}' -H 'Content-Type: application/json'
...
* *gray_list*  - array of offline  *peer*  structure as follows:
  ...
  * *port*  - unsigned int; TCP port the peer is using to connect to Exohood network.
* *status*  - string; General RPC error code. "OK" means everything looks good. Any other value means that something went wrong.
...
* *EXO_address* - String; EXO wallet address.
...
```
$ curl -X POST http://127.0.0.1:26968/get_balance -d '{"EXO_address":"EXOk8pVxq9ENz9zt4os1QGJGACaspA7wYSPrDJZMutMxBjYdZ66JGE1SBf2Ydf466LDdYLsL51sToKsko4oG73HR95AUqH8N4R"}' -H 'Content-Type: application/json'
...
```
$ curl -X POST http://127.0.0.1:26968/get_balance -d '{ "EXO_address":"EXOk8pVxq9ENz9zt4os1QGJGACaspA7wYSPrDJZMutMxBjYdZ66JGE1SBf2Ydf466LDdYLsL51sToKsko4oG73HR95AUqH8N4R", "start_out_id": 0, "batch_size": 5, "desc": false }' -H 'Content-Type: application/json'
